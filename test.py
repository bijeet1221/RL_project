...
def clamped_step(self, action):
    ...
    # clamp both nodes into [0..valid]
    n1 = max(0, min(int(n1), valid))
    n2 = max(0, min(int(n2), valid))
...
def run_and_build(env, model, outfile="out_circuit.asc"):
    ...
    while not done and steps < 100_000:
        raw, _ = model.predict(obs, deterministic=False)
...
def main():
    ...
    model = PPO.load("ppo_circuit_model.zip")
    env   = CircuitEnv(max_components=15, value_buckets=5)

    target = load_target_response("test_graphs/target_0.csv")
    env.norm_avgs, env.norm_slopes, env.target = target[0], target[1], target

    while(env.corr < 0.8 or np.isnan(env.corr)):
        print("Current correlation:", env.corr)
